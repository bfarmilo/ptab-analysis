...
class App extends Component {
  ...
  updateChart = (event) => {
    ...
    console.log('updating chart %d', parseInt(event.target.id, 10));
    ...
    fetch(`${baseUrl}/survival`, {
      ...
      body: JSON.stringify(Object.assign({
        user: userID,
        query,
        chart: parseInt(event.target, id, 10)
      }))
    })
      .then(res => res.json())
      ...
      .then(results => {
        const chartData = this.state.chartData.map((item, idx) => {
          ...
          if (item.index === idx) {
            return item.index === 0 ?
              [{ title: `${item.title} - with Duplicates`, index: item.index, count: item.countTotal, data: item.survivalTotal }] :
              [{ title: `${item.title} - unique only`, index: item.index, count: item.countUnique, data: item.survivalUnique }]
          }
